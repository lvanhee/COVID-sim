...
plotS12StackedBarRatioInfectorInfectee <- function(df_scenario12, output_dir, one_plot) {
  
  name = "s12_stacked_bar_ratio_infector_infectee"
  ...
  df_ratio_infections <- df_scenario12 %>% filter(current_governmental_model_phase != "crisis-not-acknowledged") %>%
    group_by(current_governmental_model_phase, experiment_name) %>% 
  ...
  df_list_ratio_infections = split(df_ratio_infections, f = list(df_ratio_infections$current_governmental_model_phase, df_ratio_infections$experiment_name))
  ...
  for (i in 1:length(df_list_wide_ratio_infections)) {
    ...
    t_w_r_i <- t_w_r_i %>% 
      select(current_governmental_model_phase:raw_summed_ratio) %>% 
    ...
    if (nrow(t_w_r_i) > 0) {
      # Add non-empty dataframe to list
  ...
  df_plot_all <- Reduce(rbind, df_list_wide_ratio_infections_real)
  
  for (strat in unique(df_plot_all$experiment_name)) {
      df_plot <- filter(df_plot_all, experiment_name == strat)
      
      # Add label_ypos
      df_plot <- ddply(df_plot, .(infectee, current_governmental_model_phase), transform, label_ypos= cumsum(real_ratio) - 0.5*real_ratio)
      
      p_title = paste("Ratio infectors per infectee - exit strategy: ", strat, sep="")
      dmfPdfOpen(output_dir, paste("s12_ratio_infector_infectee_strat_", strat, sep=""), p_height=6)
...
plot_ggplot_bar <- function(data_to_plot, p_title = "None") {
  ...
  (data_to_plot %>% ggplot(aes(x=infectee, y=real_ratio, fill=infector)) +
    ...
    facet_wrap(vars(current_governmental_model_phase), ncol=2) + 
    gl_plot_theme) %>% tag_facet()
}